...
int main() {
    ...
    for (int i = 0; i < 8; ++i) {
        b=0; //b se cambia a 0 cada vez que se completa el bucle de b
        a++; //Se aumenta el valor de a
        //Bucle for para incrrementar el valor de b
        for (int j = 0; j < 15; ++j) {
            b++;//Se aumenta el valor de b
            std::string operacion = std::to_string(sqrt(pow(a,2) + pow(b,2)));//Se cambia la operación de c para poder confirmar que es un número entero
            //Se confirma que pot es un número entero
            if (operacion.find(".000") != std::string::npos) {
                ...
                if((a+b)>c && (b+c) >a && (a+c) > b){
                    ...
                    std::cout << "Opción número " << opcion << "\n"
                    << "a: " << a << "\n"
                    << "b: " << b << "\n"
                    << "c: " << c
                    <<std::endl;
                    opcion ++;//Se suma 1 a opcion para que a la siguiente repetición sea una opcióm mayor
